...
<head>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .search-row { display: flex; gap: 10px; margin-bottom: 22px; align-items: center; }
    .search-row input { padding: 10px 13px; border: 1.5px solid #dfe6e9; border-radius: 6px; font-size: 14px; color: #2D3436; width: 240px; transition: border-color 0.2s; }
    .search-row input:focus { border-color: #00B894; outline: none; box-shadow: 0 0 0 3px rgba(0,184,148,0.1); }
    .search-row input::placeholder { color: #b2bec3; }

    .btn { background: #00B894; color: #fff; border: none; padding: 10px 22px; border-radius: 6px; cursor: pointer; font-size: 14px; font-weight: 600; transition: background 0.2s; text-decoration: none; display: inline-block; box-shadow: 0 4px 10px rgba(0,184,148,0.25); }
    .btn:hover { background: #00a381; }
    .btn-secondary { background: #2D3436; box-shadow: none; }
    ...
    td { padding: 11px 14px; border-bottom: 1px solid #f1f2f6; color: #2D3436; }
    ...
    .empty { color: #b2bec3; margin-top: 20px; font-size: 15px; text-align: center; padding: 40px 0; }
  </style>
</head>
<body>
...
<div class="container">
  ...
  <p class="subtitle">View and search all registered students</p>
  ...
  <form action="DisplayStudentsServlet" method="get" class="search-row">
    <input type="number" name="studentID" min="1" placeholder="Search by Student ID">
    <button type="submit" class="btn">Search</button>
    <a href="DisplayStudentsServlet" class="btn btn-secondary">View All</a>
  </form>
  ...
  %>
    <p class="count">Showing <strong><%= students.size() %></strong> record(s)</p>
    <table>
      <thead>
        <tr>
